...
def run_iterations_with_occassional_clustering(epochs, pid_batch_size=10, img_per_pid=10, experiment_id=2, warmup_epochs=5):
    ...
    best_img_model, best_text_model = start_pretraining_NC(base_image_model, bert_model, train_dloader,\
                                                    val_loader=val_dloader, device=device, model_save_path=model_save_path, epochs=warmup_epochs, img_per_pid=img_per_pid)

    torch.save(best_img_model.module.state_dict(), os.path.join(model_save_path,'best_img_model_warmup.pth'))
    torch.save(best_text_model.module.state_dict(), os.path.join(model_save_path,'best_text_model_warmup.pth'))
    ...
    ## begin clustering ##
    run_full_clustering_step(df, model_save_path, device, pid_percent_cluster=0.01)
    ## end clustering ##
    ...
    wsi_batch_dataset = WSIBatchedDataset(df, dtype='train', tokenizer=tokenizer, \
                                        img_transform=train_transform, pid_batch_size=pid_batch_size)
    train_dloader = torch.utils.data.DataLoader(wsi_batch_dataset,batch_size=1, shuffle=True, num_workers=1, pin_memory=True)

    wsi_val_dataset = WSIBatchedDataset(df, dtype='test', tokenizer=tokenizer, \
                                        img_transform=val_transform, pid_batch_size=pid_batch_size)
    val_dloader = torch.utils.data.DataLoader(wsi_val_dataset,batch_size=1, shuffle=True, num_workers=1, pin_memory=True)


    best_img_model, best_text_model = start_pretraining_warmup(best_img_model, best_text_model, train_dloader,\
                                                    val_loader=val_dloader, device=device, epochs=epochs)

    

def run_full_clustering_step(df, model_save_path, device, pid_percent_cluster=0.01):
    pids = df.pid.unique()
    ...
    base_image_model = ResnetPreTrained()
    base_image_model.load_state_dict(torch.load(os.path.join(model_save_path,'best_img_model_warmup.pth')))
    image_encoder = nn.DataParallel(ImageEncoder(base_image_model))
    image_encoder.to(device)
    ...
    cluster_all_patches(image_encoder, kmeans, gcluster_loader, device, df, save_path='/home/ss4yd/image_captioning/medcapv4/', ncentroids=8)